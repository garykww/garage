...
back to whoever dispatched it. Any of the five agents in this project
(`lead`, `inventory-code-agent`, `billing-code-agent`, `security-auditor`,
`qa-agent`) is, on its own, just a subagent. Nothing stops you from
...
Two independent modules, each with its own owner, plus a shared backlog:
...
├── logs/                          # one append-only log per agent
...
| `inventory-code-agent` | `inventory/` only | Read, Grep, Glob, Write, Edit, Bash |
| `billing-code-agent` | `billing/` only | Read, Grep, Glob, Write, Edit, Bash |
| `security-auditor` | Nothing — shared, read-only, works across both modules | Read, Grep, Glob, Bash |
| `qa-agent` | Nothing — shared, works across both modules, but only `*_test.go` files | Read, Grep, Glob, Bash, Edit, Write |
...
Every agent keeps its own append-only log under `logs/<agent-name>.md` —
findings, decisions, the plan it followed, and (per `logs/README.md`'s
shared format) a **Communication** block whenever the task involved another
agent. Each agent only ever writes to its own file, the same ownership
...
channel relaying between the two coding agents on cross-module work.
...
`billing-code-agent` each built against it.
...
`lead` reads the open tasks: four are tagged `[billing]` and dispatch
straight to `billing-code-agent`, and one — the checkout task — is tagged
`[inventory+billing]`, which neither owner can take as-is.

You can also go straight to one owner without the lead:

> Have billing-code-agent fix the issues in billing/billing.go.
