...
# Adapted from run-gemma4-26b-a4b-spark.sh; see that script for extended
# design-decision notes shared by both launchers.
#
# Usage: bash serve.sh <model> [vllm serve flags...]
#
# The image's ENTRYPOINT is the NVIDIA wrapper; `vllm serve` is passed
# explicitly as the command followed by model + flags.
#
...
#                  restarts so the ~35s compile is paid once, not every boot
...
# ---- Launch ------------------------------------------------------------------
# Remove any existing container with the same name so docker run doesn't
# conflict (mirrors the re-run behaviour of run-gemma4-26b-a4b-spark.sh).
...
# docker run flags (mirrors run-gemma4-26b-a4b-spark.sh):
