...
A showcase of a **team built entirely from Claude Code subagents**: custom
subagents defined in `.claude/agents/`, each scoped to a role with its own
system prompt and its own tool grant, coordinated by a task list and a lead
agent instead of one generalist doing everything in a single
undifferentiated context.

> **Naming note.** This folder used to be called `agent-teams`, back when
> "agent team" was just a description of the convention it demonstrates.
> Claude Code now ships a native, experimental feature with that exact name
> — [agent teams](https://code.claude.com/docs/en/agent-teams)
> (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`), with real teammate sessions, a
> shared task list, and peer-to-peer messaging. To keep the two apart, this
> folder is now `subagent-teams` (the do-it-yourself convention, built on
> the subagent primitive alone) and the sibling app
> [`agent-teams-native/`](../agent-teams-native/) showcases the native
> feature. Reading the two side by side is the point: several things this
> README documents as *impossible by construction* — module owners talking
> to each other, a machine-managed task list, enforced quality gates — are
> exactly what the native feature adds.
...
## Subagents, this team, and native agent teams
...
A **team of subagents** — what this project demonstrates — is what you get
when several subagents are given a structure that only makes sense in
relation to each other: fixed ownership
...
A **native agent team** is the third tier: the experimental Claude Code
feature of that name, which turns the convention into infrastructure. The
main session becomes the lead, teammates are full independent Claude Code
sessions (not one-shot dispatches), a shared task list with dependencies
and file-locked claiming replaces the markdown backlog, and teammates
message each other directly over a mailbox instead of relaying everything
through the lead. The structural constraint this whole project is designed
around — a subagent can only report back to whoever dispatched it, so the
lead is the sole channel between owners — simply doesn't exist there. See
[`agent-teams-native/`](../agent-teams-native/) for the same kind of
codebase run that way, and the [official docs](https://code.claude.com/docs/en/agent-teams)
...
directly is faster and has nothing extra to get wrong. And when the work
would benefit from the owners actually talking to each other — negotiating
a contract, challenging each other's findings — neither column above can do
it; that's where the native feature in
[`agent-teams-native/`](../agent-teams-native/) picks up.
...
This procedure exists *because* subagents have no channel to each other.
Under the native agent-teams feature the same task plays out differently:
the two owners are spawned as teammates and negotiate the contract directly
over `SendMessage`, with the lead reviewing the result instead of designing
it. [`agent-teams-native/`](../agent-teams-native/) stages exactly that
scenario so you can compare the two transcripts.
...
It also matters as a baseline. Everything here is built from one primitive
plus discipline, so every coordination mechanism is legible — you can read
the routing rules, the ownership boundaries, and the contract-splitting
procedure as plain markdown, and watch in `logs/` exactly where the
discipline held or slipped. The native feature in
[`agent-teams-native/`](../agent-teams-native/) replaces most of that
discipline with machinery (a real task list, real peer messaging, hooks
that actually block). Running both makes it obvious what the machinery
buys — and what the convention version already got you for free.
